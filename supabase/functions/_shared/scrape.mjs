...
async function yt(env, path, params) {
  ...
  const remaining = (env.deadline || Date.now()+15000) - Date.now();
  if(remaining <= 0) throw new Error('Collection time budget reached; remaining sources will be revisited.');
  const r = await fetch(url.toString(), {signal:AbortSignal.timeout(Math.min(15000,remaining))});
  if (!r.ok) {
...
async function searchVideoIds(env, query, maxResults = 12, order = 'relevance') {
...
function passesNicheFilter(video, thresholds) {
  ...
  if (thresholds.niche_only && !ALLOWED_CATEGORY_IDS.has(s.categoryId)) return false;

  const title       = s.title || '';
  const channel     = s.channelTitle || '';
  const description = (s.description || '').slice(0, 600);

  if (thresholds.niche_only && BLOCKLIST_REGEX.test(title))       return false;
  if (thresholds.niche_only && BLOCKLIST_REGEX.test(channel))     return false;
  if (thresholds.niche_only && BLOCKLIST_REGEX.test(description)) return false;
...
// Rotate a bounded subset so each run explores different topics and seed channels.
export function rotate(items, count, tick) {
  if (!items.length) return [];
  const size = Math.min(items.length, Math.max(0, count));
  return Array.from({length:size}, (_,i)=>items[((tick * size + i) % items.length + items.length) % items.length]);
}
export function selectDiverse(candidates, limit, perChannel=2) {
  if (limit <= 0 || perChannel <= 0) return [];
  const sorted=[...candidates].sort((a,b)=>(b.outlierScore||0)-(a.outlierScore||0));
  const counts=new Map(), seen=new Set(), hashes=new Set(), result=[];
  for(const item of sorted) {
    const channel=item.channelId||item.channel||item.id;
    if(seen.has(item.id)||(item.imageHash&&hashes.has(item.imageHash))||(counts.get(channel)||0)>=perChannel) continue;
    seen.add(item.id);if(item.imageHash)hashes.add(item.imageHash);
    counts.set(channel,(counts.get(channel)||0)+1);result.push(item);
...
export async function runScrape(env, sources, excludeIds, blockedChannelIds = new Set()) {
  env = {...env,deadline:Date.now()+100000};
  ...
  const tick = Math.floor(Date.now() / (6 * 3600000));
  const queryGroups = sources.query_groups || { niche: sources.queries || sources.search_queries || [] };
  const queries = Object.values(queryGroups).flatMap(group => rotate(group, sources.queries_per_group || 3, tick));
  const channels = rotate(sources.channels || [], sources.channels_per_run || 10, tick);
  stats.search_queries = queries;
  stats.seed_channels = channels;
  ...
  for (const q of queries) {
    try {
      const ids = await searchVideoIds(env, q, 12, ['relevance','date','viewCount'][tick % 3]);
      for (const id of ids) collected.add(id);
    } catch (e) {
  ...
  for (const cat of rotate(['1', '17', '24', '26', '27', '28'], 3, tick)) {
    try {
      const ids = await trendingVideoIds(env, cat, rotate(sources.regions || ['US','GB','ES'], 1, tick)[0], 25);
      for (const id of ids) collected.add(id);
    } catch (e) {
  ...
  const toScore = rotate(notShorts, Math.min(120, sources.max_candidates_per_run || 120), tick);
  const channelIds = [...new Set(toScore.map(v=>v.snippet?.channelId).filter(Boolean))];
  let nextChannel = 0;
  await Promise.all(Array.from({length:6}, async()=>{
    while(nextChannel < channelIds.length) await getChannelBaseline(env,channelIds[nextChannel++],baselineCache);
  }));
  stats.scored_candidates = toScore.length;
  const scored = [];
  ...
  const capped = selectDiverse(scored, thresholds.cap_per_run, sources.max_per_channel || 2);
